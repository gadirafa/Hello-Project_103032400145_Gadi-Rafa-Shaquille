...
func insertionSort(data *TabPinjaman, n int) {
	for i := 1; i < n; i++ {
		key := data[i]
		j := i - 1
		for j >= 0 && compareString(data[j].NamaPeminjam, key.NamaPeminjam) > 0 {
			data[j+1] = data[j]
			j--
		}
		data[j+1] = key
...
func menuSearching(data TabPinjaman, n int) {
	...
	case 2:
		insertionSort(&data, n) // Gunakan sorting manual
		idx := binarySearch(data, n, target)
		if idx != -1 {
			fmt.Println("Data ditemukan:")
			tampilkanPinjaman(data[idx])
		} else {
...
func sequentialSearch(data TabPinjaman, n int, target string) int {
	for i := 0; i < n; i++ {
		if compareString(data[i].NamaPeminjam, target) == 0 {
			return i
		}
	}
	return -1
...
func binarySearch(data TabPinjaman, n int, target string) int {
	...
	for low <= high {
		mid := (low + high) / 2
		comp := compareString(data[mid].NamaPeminjam, target)
		if comp == 0 {
			return mid
		} else if comp < 0 {
			low = mid + 1
...
// Fungsi pembanding string manual tanpa tolower
func compareString(a, b string) int {
	...
	for i := 0; i < minLen; i++ {
		ca, cb := a[i], b[i]
		// Jika huruf kapital, ubah manual ke huruf kecil
		if ca >= 'A' && ca <= 'Z' {
			ca += 32
		}
		if cb >= 'A' && cb <= 'Z' {
